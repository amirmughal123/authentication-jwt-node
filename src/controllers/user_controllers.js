...
const transporter = nodemailer.createTransport({
  ...
  requireTLS: true,
  auth: {
    user: bcryptSaltRounds.EMAIL,
    pass: bcryptSaltRounds.PASSWORD
  }
});
    ...
    if(!user) {
      transporter.sendMail(mailOptions, async(error, info) => {
        if (error) {
          ...
          return res.send({ status: false, msg: 'Please use your correct email for registration' });
        } else {
...
export default { create, getAll, login, resetPassword };
